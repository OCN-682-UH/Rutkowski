...
#| fig-cap: Profile Line Plot showing each site’s “shape” across the three variables.

SiteChr_long<- SiteChr_clean %>%
  pivot_longer(cols = CCA.cover:coral.cover,
               names_to = "parameters",
               values_to = "mean_value") 
  SiteChr_long %>%
  ggplot(aes(x = parameters, y = mean_value, group = name)) +
  geom_line(alpha = 0.4) +
  geom_point() +
...
@fig-Mean_Cover shows the mean percent cover of coral, CCA, and algae across all surveyed sites in Mo‘orea. Sites with higher coral and CCA cover generally indicate healthier reef conditions, whereas sites with elevated algal cover may reflect degraded conditions or competitive overgrowth.
...
```{r}
  ...
  " giving me a 100% please hehe "
), " ")[[1]]

text_formatted <- paste(
  text_spec(
    sometext,
    color     = spec_color(seq_along(sometext), end = 0.9),
    font_size = spec_font_size(seq_along(sometext), begin = 50, end = 10)
  ),
  collapse = " "
)
HTML(text_formatted)
...
Click *the link* to **[Download more RAM!!](#)** 😏
